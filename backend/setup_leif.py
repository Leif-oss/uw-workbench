"""
Standalone script to set up Leif as an admin user.
Run this from the project root: python -m backend.setup_leif
Or from backend directory: python setup_leif.py (if PYTHONPATH includes parent)
"""
import sys
import os
...
def setup_leif():
    """Create or update Leif as an admin user with both Employee and User accounts."""
    ...
    try:
        ...
        db.rollback()
        print(f"\n❌ ERROR: Failed to set up admin user: {e}")
        import traceback
        traceback.print_exc()
        raise
    finally:
